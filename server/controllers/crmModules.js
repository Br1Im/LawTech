...
const materials = {
  async list(req, res) {
    try {
      const officeId = req.params.officeId || req.user.office_id;
      const [rows] = await db.query(
        `SELECT m.*, ca.title AS case_title
         FROM materials m
...
const employees = {
  ...
  async create(req, res) {
    try {
      ...
      const [r] = await db.query(
        `INSERT INTO employees (first_name, last_name, email, phone, position, office_id)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [first_name, last_name, email || null, phone || null, position || 'Юрист', officeId]
      );
  ...
  async update(req, res) {
    try {
      const { id } = req.params;
      const fields = ['first_name','last_name','email','phone','position','office_id'];
      const updates = fields.filter(f => req.body[f] !== undefined);
      if (updates.length === 0) return bad(res, 400, 'Нет полей для обновления');
      const setSql = updates.map(f => `${f} = ?`).join(', ');
      const values = updates.map(f => req.body[f]);
